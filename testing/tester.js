...
function getNavPosition(position) {
    $('#nav_location').html((Math.round(position.coords.latitude * 10000) / 10000)+','+ (Math.round(position.coords.longitude * 10000) / 10000));
    $('#nav_map').html('<img src="http://api.tiles.mapbox.com/v4/'+mapbox_project_sat_streets+'/'+position.coords.longitude+','+position.coords.latitude+',14/500x100.png32?access_token='+mapbox_token+'" />');
    $.getJSON('http://nominatim.openstreetmap.org/reverse?format=json&lat='+position.coords.latitude+'&lon='+position.coords.longitude, function (result, textStatus, jqXHR) {
    	$('#nav_address').html(result.display_name);
    	});
	}
...
function getMyLocation() {
	$.getJSON('http://freegeoip.net/json/?callback=?', function (result, textStatus, jqXHR) {
        ...
        $('#my_ip').html(my_location.ip);
      	...
      	$('#my_map').html('<img src="http://api.tiles.mapbox.com/v4/'+mapbox_project_sat_streets+'/'+my_location.longitude+','+my_location.latitude+',14/500x100.png32?access_token='+mapbox_token+'" />');
      	});	
	}
...
function getBrowsingLocation(iphostname) {
	$.getJSON('http://freegeoip.net/json/'+iphostname+'?callback=?', function (result, textStatus, jqXHR) {
	    ...
	    $('#browsing_ip').html(browsing_location.ip);
      	...
      	$('#browsing_map').html('<img src="http://api.tiles.mapbox.com/v4/'+mapbox_project_sat_streets+'/'+browsing_location.longitude+','+browsing_location.latitude+',14/500x100.png32?access_token='+mapbox_token+'" />');
		});
	}